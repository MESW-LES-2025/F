...
export function PantryGrid({ items, pantryHouseId, pantryId }: PantryGridProps) {
            ...
            <Card key={item.id} className="p-4 bg-white border border-gray-200 hover:shadow-md transition-shadow">
              <div className="flex items-start justify-between mb-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button className="p-1 rounded hover:bg-gray-100" aria-label="Edit item" onClick={() => {
                    setDialogItem(item)
                    setFormName(item.name ?? "")
                    setFormUnit(item.unit ?? "UNITS")
                    setFormCategory(item.category ?? "OTHER")
                    setFormQuantity(item.quantity ?? 0)
                    setFormExpiry(item.expiryDate ? (item.expiryDate as Date).toISOString().slice(0,10) : "")
                    setDialogOpen(true)
                  }}>
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      onClick={async () => {
                        if (!pantryHouseId || !pantryId) {
                          toast({ title: 'Pantry not available', description: 'Cannot update quantity: pantry id missing.' })
                          return
                        }
                        ...
                        try {
                          await apiPatch(`/pantry/${pantryHouseId}/${pantryId}`, { items: [{ itemId: item.id, quantity: newQty }] }, { requiresAuth: true })
                          toast({ title: 'Quantity updated', description: 'Item quantity decreased.' })
                        } catch (err) {
                    ...
                    <span className="font-medium text-gray-900">
                      {editingId === item.id ? (
                        <div className="flex items-center gap-2">
                          ...
                          <UiButton size="sm" onClick={async () => {
                            ...
                            if (Number.isNaN(parsed) || parsed < 0) {
                              toast({ title: 'Invalid quantity', description: 'Please enter a valid non-negative number.' })
                              return
                            }
                            const originalQty = item.quantity
                        ...
                        <button className="text-gray-900" onClick={() => { setEditingId(item.id); setEditingValue(item.quantity) }}>
                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        try {
                          await apiPatch(`/pantry/${pantryHouseId}/${pantryId}`, { items: [{ itemId: item.id, quantity: newQty }] }, { requiresAuth: true })
                          toast({ title: 'Quantity updated', description: 'Item quantity increased.' })
                        } catch (err) {
                          // revert
                          setLocalItems((prev) => prev.map((it) => it.id === item.id ? { ...it, quantity: originalQty, lowStock: Number(originalQty) <= 1 } : it))
                          toast({ title: 'Update failed', description: String(err) })
  ...
  <Dialog open={dialogOpen} onOpenChange={(open) => { if (!open) { setDialogOpen(false); setDialogItem(null) } else setDialogOpen(true) }}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Change fields and save. You can also remove this item from the pantry.</DialogDescription>
          </DialogHeader>
          ...
          {dialogItem && (
            <div className="space-y-4">
              ...
              <div>
                ...
                <TextInput type="number" value={String(formQuantity)} onChange={(e) => setFormQuantity(e.target.value === "" ? "" : Number(e.target.value))} />
              </div>
              ...
              <div>
                ...
                <TextInput type="date" value={formExpiry} onChange={(e) => setFormExpiry(e.target.value)} />
              </div>
            </div>
          )}
            ...
            <div className="flex items-center justify-between w-full">
              <div className="flex gap-2">
                <UiButton variant="destructive" onClick={async () => {
                  ...
                  try {
                    await apiDelete(`/pantry-item/${dialogItem.id}`, { requiresAuth: true })
                    toast({ title: 'Deleted', description: 'Item removed from catalog and all pantries.' })
                    ...
                    const status = err?.status ?? (err?.originalError as any)?.status
                    if (status === 403 || status === 404) {
                      // try removing only the pantry association
                      if (!pantryHouseId || !pantryId) { toast({ title: 'Pantry missing', description: 'Cannot remove item - pantry id missing.' }); return }
                      try {
                        await apiPatch(`/pantry/${pantryHouseId}/${pantryId}`, { items: [{ itemId: dialogItem.id, quantity: 0 }] }, { requiresAuth: true })
                        toast({ title: 'Partially removed', description: 'Item removed from this pantry (catalog not deleted: insufficient permissions).' })
                        setLocalItems((prev) => prev.filter((it) => it.id !== dialogItem.id))
                        setDialogOpen(false)
                        return
                      } catch (err2) {
                        toast({ title: 'Remove failed', description: String(err2) })
                        return
                      }
                    }
              ...
              <div className="flex gap-2">
                <UiButton variant="ghost" onClick={() => { setDialogOpen(false); setDialogItem(null) }}>Cancel</UiButton>
                <UiButton onClick={async () => {
                  ...
                  try {
                    ...
                    toast({ title: 'Catalog update failed', description: String(err) })
                  }
                  try {
                    if (pantryHouseId && pantryId) {
                      ...
                      setLocalItems((prev) => prev.map((it) => it.id === dialogItem.id ? { ...it, name: formName, category: formCategory, unit: formUnit, quantity: Number(formQuantity), expiryDate: formExpiry ? new Date(formExpiry) : undefined, lowStock: Number(formQuantity) <= 1 } : it))
                    }
                  } catch (err) {
