  ...
  const handleSettle = async () => {
    ...
    try {
      ...
      const payload: CreateExpensePayload = {
        ...
        description:
          description ||
          `Settlement: ${fromMember?.name} pays ${toMember?.name}`,
        category: "SETTLEMENT",
        paidById: fromUser,
        ...
        <div className="space-y-4 py-4">
          ...
          <div className="rounded-lg bg-blue-50 dark:bg-blue-950 p-3 text-sm">
            ...
            <p className="text-blue-700 dark:text-blue-200 text-xs">
              This will be recorded as a payment from{" "}
              {selectedFromMember?.name || "the payer"} to{" "}
              {selectedToMember?.name || "the receiver"}, adjusting both
              members&apos; balances accordingly.
            </p>
          </div>
        </div>
