  ...
  const filtered = useMemo(() => {
    ...
    return list.filter((it) => {
      ...
      if (addedBy === "me") {
        ...
        if (it.addedBy.toLowerCase().includes(currentUserName) === false) return false
      } else if (addedBy === "others") {
        if (currentUserName && it.addedBy && it.addedBy.toLowerCase().includes(currentUserName)) return false
      }
      ...
      if (status === "low-stock") {
        ...
        const days = Math.ceil((it.expiryDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24))
