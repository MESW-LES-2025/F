...
export function ExpensesStats({ houseId, refreshTrigger }: ExpensesStatsProps) {
  ...
  if (isLoading) {
    return (
      <div className="grid grid-cols-4 gap-4">
        {[1, 2, 3, 4].map((i) => (
          <Card key={i} className="p-4 bg-white border border-gray-200 flex items-center justify-center">
  ...
  const topSpender = Object.entries(expensesByPerson).length > 0
  ...
  return (
    <div className="grid grid-cols-4 gap-4">
      ...
      <Card className="p-4 bg-white border border-gray-200">
        ...
        <p className="text-2xl font-bold text-gray-900">{topSpender ? topSpender[0] : '-'}</p>
      </Card>
    </div>
  )
}
