...
export default function ExpensesPage() {
  ...
  const [sortField, setSortField] = useState<"date" | "amount" | "payer">(
    "date",
  );
          ...
          <div className="flex items-center justify-center h-64">
            <div className="text-center">
              ...
              <p className="text-sm text-gray-400">
                Please select a house to view expenses
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="p-4 md:p-6 lg:p-8 space-y-6">
        ...
        <Tabs defaultValue="overview" className="space-y-6">
          <TabsList className="grid w-full grid-cols-4">
            ...
            <TabsTrigger value="balances" className="cursor-pointer">
              Balances
            </TabsTrigger>
          ...
          <TabsContent value="overview" className="space-y-6">
            <div className="grid gap-6 md:grid-cols-2">
              <BalancesWidget
                houseId={houseId}
                key={`balances-${refreshTrigger}`}
                onRefresh={handleExpenseCreated}
              />
              <Card>
                <CardHeader>
                  <CardTitle>Recent Expenses</CardTitle>
                  <CardDescription>
                    Latest transactions in your household
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <ExpensesList
          ...
          <TabsContent value="balances" className="space-y-6">
            <BalancesWidget
              houseId={houseId}
              key={`balances-detail-${refreshTrigger}`}
              onRefresh={handleExpenseCreated}
            />
          </TabsContent>
          ...
          <TabsContent value="analytics" className="space-y-6">
            <SpendingTrendsChart
              houseId={houseId}
              key={`trends-analytics-${refreshTrigger}`}
            />
